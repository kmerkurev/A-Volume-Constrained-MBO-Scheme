...
void run_special_auction_phase(s_heap *heapHolder, float *weights, float *prices, int *labels, int *unassigned, int *lowerVolumes, int *upperVolumes, int pcount, int lcount, int *unassignedCount, float epsilon){
    int i,l;
    int uc=0;
    for(i=0;i<(*unassignedCount);i++){
        int index=unassigned[i];
        ...
        float bid=prices[bidItem]+epsilon+bestValue-nextBest;
        if(heapHolder[bidItem].count==upperVolumes[bidItem]){
            int minIndex=heapHolder[bidItem].root[0].originalIndex;
            labels[minIndex]=-1;
            heapHolder[bidItem].root[0].key=bid;
            heapHolder[bidItem].root[0].originalIndex=index;
            ...
            prices[bidItem]=heapHolder[bidItem].root[0].key;
            uc++;
        }else if(heapHolder[bidItem].count==lowerVolumes[bidItem]&&prices[bidItem]<0){
            int minIndex=heapHolder[bidItem].root[0].originalIndex;
            labels[minIndex]=-1;
            heapHolder[bidItem].root[0].key=bid;
            heapHolder[bidItem].root[0].originalIndex=index;
            ...
            labels[index]=bidItem;
            unassigned[uc]=minIndex;
            prices[bidItem]=fmin(heapHolder[bidItem].root[0].key,0);
            uc++;
...
void run_special_reverse_auction_phase(s_heap heap, float *weights, unsigned char *fixedLabels, float *prices, int *labels, int *lowerVolumes, int *upperVolumes, int *currentVolumes, int pcount, int lcount,  float epsilon){
    int i,l;
    for(l=0;l<lcount;l++){
        int numMissing=lowerVolumes[l]-currentVolumes[l];
        int upperDistance=upperVolumes[l]-currentVolumes[l];
        if(numMissing>0){
            heap.count=0;
            for(i=0;i<pcount;i++){
                if(labels[i]!=l&&!fixedLabels[i]){
        ...
        if(upperDistance>0&&prices[l]>0){
            ...
            float priceSlash=heap.root[0].key;
            if(prices[l]+priceSlash-epsilon<=0){
                for(i=0;i<upperDistance;i++){
                    if(prices[l]+heap.root[i].key>epsilon){
                        int index=heap.root[i].originalIndex;
                        currentVolumes[labels[index]]--;
                        currentVolumes[l]++;
                        labels[index]=l;
...
void threshold(float *weights, float *prices, int *labels, unsigned char *fixedLabels, int *lowerVolumes, int *upperVolumes, int *currentVolumes, int pcount, int lcount, int *feasible){
    int i,l;
    memset(currentVolumes,0,lcount*sizeof(int));
    for(i=0;i<pcount;i++){
        if(!fixedLabels[i]){
            ...
            int ml=0;
            for(l=0;l<lcount;l++){
                if(weights[lcount*i+l]-prices[l]>max){
                    max=weights[lcount*i+l]-prices[l];
                    ml=l;
                }
            }
            labels[i]=ml;
    ...
    for(l=0;l<lcount;l++){
        if(currentVolumes[l]>upperVolumes[l]||currentVolumes[l]<lowerVolumes[l]){
            *feasible=0;
        }else if(prices[l]>0&&currentVolumes[l]<upperVolumes[l]){
            *feasible=0;
        }else if(prices[l]<0&&currentVolumes[l]>lowerVolumes[l]){
            *feasible=0;
        }
    ...
    while(epsilon>epsilonMin){
        ...
        while(!feasible){
            ...
            while(!done){
                ...
                run_special_auction_phase(forwardHeapHolder, weights, prices, labels, unassigned, lowerVolumes, upperVolumes, pcount, lcount, &unassignedCount, epsilon);
                numBids+=unassignedCount;
                if(unassignedCount>0){
                    done=0;
                    
                    for(l=0;l<lcount;l++){
                        if(currentVolumes[l]>lowerVolumes[l]&&prices[l]<0){
                            done=0;
                        }
            ...
            epsilon=fmax(epsilonMin, epsilon/scalingFactor);
            for(l=0;l<lcount;l++){
                if((currentVolumes[l]<lowerVolumes[l])||(currentVolumes[l]<upperVolumes[l]&&prices[l]>0)){
                    feasible=0;
                }
            }
            done=0;
            if(!feasible){
                ...
                while(!done){
                    run_special_reverse_auction_phase(reverseHeap, weights, fixedLabels, prices, labels, lowerVolumes, upperVolumes, currentVolumes, pcount, lcount, epsilon);
                    done=1;
                    for(j=0;j<lcount;j++){
                        if((currentVolumes[j]<lowerVolumes[j])||(currentVolumes[j]<upperVolumes[j]&&prices[j]>0)){
                            done=0;
                        }
                    }
                }
        ...
        if(epsilon>epsilonMin){
            threshold(weights, prices, labels, fixedLabels, lowerVolumes, upperVolumes, currentVolumes, pcount,  lcount, &feasible);
            if(feasible){
                break;
            }
        }
...
float update_labels(mbo_struct mbos, float *convLabels, int *volumes, int *cc){
    int i,l;
    float upper_bound= mbos.upper_bound;
    float lower_bound= mbos.lower_bound;
    int pcount=mbos.pcount;
    int lcount=mbos.lcount;
    int *labels=mbos.labels;
    unsigned char *fixedLabels=mbos.fixedLabels;
    ...
    if(surfaceTensions){
        ...
        for(l=0;l<lcount;l++){
            lowerVolumes[l]= volumes[l]*lower_bound;
            upperVolumes[l]= volumes[l]*upper_bound;
            if (lower_bound>10){
                lowerVolumes[l]= 0;
                upperVolumes[l]= pcount;
            }
        }
